          ...
          <div class="preview-box">
            ...
            <div class="image-container">
              <div class="comparison-slider" v-if="!loading && compressProgress>=100">
                <img :src="compressedPreview" alt="Compressed" class="comparison-image" />
                <div class="comparison-overlay" :style="{ '--position': comparisonValue + '%' }">
                  <img :src="originalPreview" alt="Original" class="comparison-image" />
                </div>
            ...
            <div class="image-info">
              <div class="image-info-item">
                ...
                <span v-if="compressedSize && originalSize" class="compression-ratio" :class="{ 'good-ratio': (compressedSize / originalSize) < 0.6 }">
...
// 监听宽度变化，自动调整高度
watch(() => options.maxWidth, (newWidth, oldWidth) => {
  if (options.maintainRatio && originalDimensions.width > 0 && newWidth !== oldWidth) {
...
// 监听高度变化，自动调整宽度
watch(() => options.maxHeight, (newHeight, oldHeight) => {
  if (options.maintainRatio && originalDimensions.width > 0 && newHeight !== oldHeight) {
...
const compressImageWithCanvas = async (file, quality = 0.6, maxWidth = 1920, maxHeight = 1080) => {
  ...
  try {
    ...
    let compressedBlob = await compressImageWithCanvas(
      ...
      options.maxHeight
		...
		console.log(compressedBlob.size,originalSize.value)
...
const downloadImage = async () => {
  try {
    ...
    const blob = new Blob(byteArrays, { type: imageFile.value.type || 'image/jpeg' })
...
.background-pattern {
  ...
  bottom: 0;
  background:
    radial-gradient(circle at 10% 20%, rgba(200, 230, 201, 0.2) 0%, transparent 40%),
    radial-gradient(circle at 90% 80%, rgba(187, 222, 251, 0.2) 0%, transparent 40%);
  pointer-events: none;
  z-index: 0;
}
...
.comparison-overlay {
  ...
  border-right: 2px solid rgba(179, 157, 219, 0.5);
}
